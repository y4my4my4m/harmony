-- Migration 081: Fix Federation Queue Data Types
-- ISSUE: activity_id column is UUID but we're passing TEXT
-- SOLUTION: Generate UUID for activity_id, put ActivityPub ID in activity_data
    ...
    IF NEW.conversation_id IS NOT NULL AND sender_profile.is_local THEN
        ...
        IF v_instance_domain IS NOT NULL THEN
            ...
            LOOP
                ...
                -- Generate UUID for database activity_id
                ...
                -- ✅ USE CORRECT DATA TYPES for federation_delivery_queue
                INSERT INTO federation_delivery_queue (
                    activity_id,           -- UUID (database ID)
                    target_domain,         -- TEXT
                    target_inbox_url,      -- TEXT
                    actor_username,        -- TEXT
                    actor_domain,          -- TEXT
                    activity_data          -- JSONB (contains ActivityPub ID in 'id' field)
                ) VALUES (
                    v_activity_uuid,       -- ✅ UUID for database
                    v_recipient_profile.domain,
                    v_inbox_url,
                    sender_profile.username,
                    v_instance_domain,
                    v_activity              -- ✅ JSONB containing the full activity
                );
