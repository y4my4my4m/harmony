...
-- Replace the send_notification function to work with existing table schema
...
BEGIN
    ...
    LOOP
        ...
        IF should_send THEN
            ...
            -- Insert into existing table schema (only columns that exist)
...
COMMENT ON FUNCTION public.send_notification(character varying, uuid[], jsonb, uuid, uuid, uuid, uuid, character varying) 
IS 'SECURITY DEFINER: Unified notification function that stores contextual data in JSONB and respects user preferences.';

-- Verify the function works with existing schema
DO $$
DECLARE
    test_result uuid[];
    test_user_id uuid;
