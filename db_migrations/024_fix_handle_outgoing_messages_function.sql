-- Migration 024: Fix handle_outgoing_messages function for new conversation structure
-- 
-- ISSUE: handle_outgoing_messages() function still references old user1/user2 columns
--        causing "column c.user1 does not exist" errors when sending DM messages
-- FIX: Update function to use conversation_participants table instead of user1/user2

-- =====================================================
-- STEP 1: Drop and recreate handle_outgoing_messages function
-- =====================================================
...
DECLARE
    ...
    -- Variables for conversation participants (UPDATED FOR NEW STRUCTURE)
...
COMMENT ON FUNCTION public.handle_outgoing_messages() IS 'UPDATED: Handles outgoing message notifications and federation. Now uses conversation_participants table instead of deprecated user1/user2 columns. Supports multi-participant conversations and group chats.';
...
BEGIN
    ...
    RAISE NOTICE '  ✅ handle_outgoing_messages() function updated for new conversation structure';
    RAISE NOTICE '  ✅ Removed references to non-existent user1/user2 columns';
    RAISE NOTICE '  ✅ Now uses conversation_participants table properly';
    RAISE NOTICE '  ✅ Supports multi-participant conversations and group chats';
    RAISE NOTICE '';
    RAISE NOTICE 'DM message sending should now work in production!';
END $$;
...
-- CHANGE SUMMARY:
-- 1. Updated notification logic to use conversation_participants instead of user1/user2
-- 2. Updated federation logic to find remote recipients via conversation_participants
-- 3. Added support for multi-participant conversations
-- 4. Maintained all existing functionality while fixing column reference errors

-- COMPATIBILITY:
-- - Function signature unchanged - no breaking changes for triggers
-- - All notification types still supported (DM, reply, mention)
-- - Federation functionality preserved and enhanced
-- - Performance should be similar or better with proper indexing
