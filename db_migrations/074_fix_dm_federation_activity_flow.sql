-- Migration 074: Fix DM Federation Activity Flow
-- ISSUE: DMs failing because activity_id is NULL in federation_delivery_queue
-- ROOT CAUSE: Direct insertion to federation_delivery_queue instead of using proper flow
-- SOLUTION: Use ap_activities -> queue_activity_for_federation flow like posts

BEGIN;

-- =================================================================
-- Fix handle_outgoing_messages to use proper federation flow
-- =================================================================
...
DECLARE
    ...
    v_activity_id TEXT;
    v_activity_uuid UUID;  -- CRITICAL: This will store ap_activities.id
    v_sender_url TEXT;
    ...
    target_domains TEXT[];  -- ADDED: For queue_activity_for_federation
BEGIN
    ...
    IF NEW.conversation_id IS NOT NULL THEN
        ...
        LOOP
            ...
            IF participant_record.is_local THEN
                -- FIXED: Use the correct notification function
                PERFORM send_notification_to_user(
                    ...
                    NEW.id  -- related_object_id (message id)
                );
            END IF;
        END LOOP;
        ...
        IF v_instance_domain IS NOT NULL AND v_instance_domain != 'debug' AND v_instance_domain != '' THEN
            ...
            LOOP
                ...
                IF v_recipient_profile.domain != 'debug' AND v_recipient_profile.domain != '' THEN
                    ...
                    -- Ensure recipient is mentioned in the tags
                    IF NOT EXISTS (
                        SELECT 1 FROM jsonb_array_elements(v_tags) tag
                        WHERE tag->>'href' = v_recipient_url
                    ) THEN
                        v_tags := v_tags || jsonb_build_array(jsonb_build_object(
                            'type', 'Mention',
                    ...
                    -- Create Note object (DM format with proper addressing)
                    v_note_object := jsonb_build_object(
                        ...
                        'url', v_message_url,
                        'to', jsonb_build_array(v_recipient_url),  -- Direct addressing to recipient
                    ...
                    -- FIXED: Use proper federation flow with ap_activities table
                    ...
                    -- FIXED: Use queue_activity_for_federation like posts do
                    ...
                    RAISE WARNING '📮 Queued DM for federation to: %@% (activity: %)', 
                        v_recipient_profile.username, v_recipient_profile.domain, v_activity_uuid;
                ELSE
                    RAISE WARNING '⚠️  Skipping DM to invalid domain: %', v_recipient_profile.domain;
                END IF;
            END LOOP;
        ELSE
            RAISE WARNING '⚠️  Skipping federation - invalid instance domain: %', v_instance_domain;
        END IF;
    END IF;
