-- Fix Content Preview Extraction JSON Error
-- ISSUE: PostgreSQL is trying to parse "@username" concatenation as JSON
-- SOLUTION: Explicitly cast string concatenation to TEXT
...
BEGIN
    ...
    -- CRITICAL FIX: Extract content preview for notifications (with proper TEXT casting)
    ...
    IF NEW.conversation_id IS NOT NULL AND sender_profile.is_local THEN
        ...
        IF v_instance_domain IS NOT NULL THEN
            ...
            WHERE cp.conversation_id = NEW.conversation_id 
              ...
              AND p.federated_id IS NOT NULL;  -- CRITICAL: Only include users with valid federated_id
            ...
            IF v_all_remote_participants IS NOT NULL AND array_length(v_all_remote_participants, 1) > 0 THEN
                ...
                -- CRITICAL FIX: Use federated_id directly (it's already the complete ActivityPub actor URL)
                SELECT jsonb_agg(p.federated_id) INTO v_to_addresses
                FROM profiles p
                WHERE p.id = ANY(v_all_remote_participants)
                  AND p.federated_id IS NOT NULL;
                
                -- CRITICAL FIX: Use federated_id directly for recipient URLs array
                SELECT ARRAY_AGG(p.federated_id) INTO v_recipient_urls
                FROM profiles p
                WHERE p.id = ANY(v_all_remote_participants)
                  AND p.federated_id IS NOT NULL;
                ...
                BEGIN
                    ...
                    v_tags := extract_activitypub_mention_tags(
                        ...
                        v_recipient_urls,  -- All remote recipients (using federated_id directly)
                        v_instance_domain
                    );
