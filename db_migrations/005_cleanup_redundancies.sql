...
-- =====================================================
-- STEP 1: REMOVE REDUNDANT FEDERATION HEALTH MONITORING
-- =====================================================

-- Drop redundant tables I created (federation_stats already exists)
DROP TABLE IF EXISTS federation_health CASCADE;
DROP TABLE IF EXISTS federation_errors CASCADE;

-- Drop redundant function I created
DROP FUNCTION IF EXISTS public.log_federation_health(text, text, text, integer, integer, text, jsonb);

-- Keep only the existing federation_stats view and enhance it if needed
COMMENT ON VIEW federation_stats IS 'Existing federation statistics view - comprehensive and sufficient';

-- =====================================================
-- STEP 2: PROPERLY UNIFY NOTIFICATION FUNCTIONS
-- =====================================================

-- Create the ONE TRUE notification function that handles everything
CREATE OR REPLACE FUNCTION public.create_notification_unified(
    p_user_id uuid,
    p_type text,
...
BEGIN
    ...
    SELECT should_create_notification(p_user_id, p_type, p_server_id, p_channel_id, p_conversation_id)
    INTO v_should_create;
...
COMMENT ON FUNCTION public.create_notification_unified IS 'Unified notification function replacing all old variants. Handles all notification types with preference checking.';

-- Update the wrapper functions to use the unified one
...
AS $$
BEGIN
    RETURN create_notification_unified(p_user_id, p_type, p_title, p_message, p_data);
END;
$$;

COMMENT ON FUNCTION public.create_notification(uuid, character varying, character varying, text, jsonb) IS 'DEPRECATED: Use create_notification_unified() directly. This is a compatibility wrapper.';
...
COMMENT ON FUNCTION public.create_notification_structured(uuid, character varying, jsonb, uuid, uuid, uuid) IS 'DEPRECATED: Use create_notification_unified() directly. This is a compatibility wrapper.';

-- Update send_notification to use the unified function
...
COMMENT ON FUNCTION public.send_notification IS 'Updated to use create_notification_unified(). Sends notifications to multiple users efficiently.';
