...
// Federation backend env-schema default. Surfaced over NodeInfo and /health, so
...
// Android tauri.properties. Generated by `tauri android build` and gitignored,
// so it is absent on a fresh checkout; the build derives both fields from
// tauri.conf.json when it regenerates the file.
{
  ...
  if (existsSync(path)) {
    ...
    console.log(`${path} absent; regenerated by the Android build (versionCode ${versionCode})`);
  }
}
