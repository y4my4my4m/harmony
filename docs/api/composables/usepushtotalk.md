# usePushToTalk Composable

**File:** `src/composables/usePushToTalk.ts`

## Overview

```mermaid
graph TB
    subgraph "usePushToTalk Composable"
        INPUTMODE[InputMode]
        PTTSETTINGS[PTTSettings]
        PTTMODIFIERS[PTTModifiers]
        USEPUSHTOTALK[usePushToTalk]
    end
    
    subgraph "Functions"
        FN_USEPUSHTOTALK[usePushToTalk]
        FN_SHOULDBLOCKSHORTCUT[shouldBlockShortcut]
        FN_REGISTERMUTECALLBACK[registerMuteCallback]
        FN_UNREGISTERMUTECALLBACK[unregisterMuteCallback]
        FN_SETUPLISTENERS[setupListeners]
        FN_CLEANUPLISTENERS[cleanupListeners]
    end
    
    subgraph "Interfaces"
        INT_PTTSETTINGS[PTTSettings]
        INT_PTTMODIFIERS[PTTModifiers]
    end
```
...
- **InputMode** - type export
- **PTTSettings** - interface export
- **PTTModifiers** - interface export
- **usePushToTalk** - function export
...
### `shouldBlockShortcut(event: KeyboardEvent)`
...
### `registerMuteCallback(callback: (muted: boolean)`

No description available.

**Parameters:**
- `callback: (muted: boolean`
...
/**
   ...
   */
  const registerMuteCallback = (callback: (muted: boolean) =>
```
...
```typescript
import { InputMode, PTTSettings, PTTModifiers, usePushToTalk } from '@/composables/usePushToTalk'
