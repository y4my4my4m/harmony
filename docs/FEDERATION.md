...
[`ACTIVITYPUB_EXTENSIONS.md`](./ACTIVITYPUB_EXTENSIONS.md). This file is
...
[`ACTIVITYPUB_EXTENSIONS.md`](./ACTIVITYPUB_EXTENSIONS.md). None of it has
been put through the FEP process yet; it's a draft and the version number in that
file is the one to trust.

## Future enhancements

Federation work that is scoped but not shipped:

1. **Federated E2EE - Phase 1 (Harmony ↔ Harmony)**: extend the existing
   Megolm-style encryption across Harmony instances. Adds device-key publishing
   on AP actors, an `EncryptedMessage` activity type, Olm-style pairwise key
   sharing between remote devices, and cross-user verification UX. Depends on no
   external standard. See
   [`E2EE_IMPLEMENTATION.md` → Roadmap: Federated E2EE](./E2EE_IMPLEMENTATION.md#roadmap-federated-e2ee).
2. **Federated E2EE - Phase 2 (Harmony ↔ other AP clients)**: either implement
   the [MLS-on-ActivityPub draft](https://swicg.github.io/activitypub-e2ee/mls)
   for interop with Sup and future MLS-AP clients, or publish Harmony's Phase-1
   protocol as a [FEP](https://codeberg.org/fediverse/fep). Phase 1 is the
   prerequisite either way.
3. **Chat-server discovery by handle**: the WebFinger gap described under
   [Discovery](#webfinger).
4. **Capability negotiation via NodeInfo**: described under
   [Joining a server across instances](#joining-a-server-across-instances).

Already shipped: cross-instance voice channels via LiveKit
(`allow_federated_voice` setting, `federated_voice_calls` table) - see
`federation-backend/src/services/LiveKitService.ts`.
