...
| Variable | Description |
|----------|-------------|
| `DATABASE_URL` | PostgreSQL connection string for the `LISTEN/NOTIFY` bridge |
| `REDIS_URL` | Redis connection string (BullMQ job persistence + cache + rate limits) |
| `USE_BULLMQ_QUEUE` | `true` to enable reliable job queue (default, recommended). `USE_PGBOSS_QUEUE` is accepted as a backward-compat alias |
