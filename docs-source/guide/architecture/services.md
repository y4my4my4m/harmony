...
Services write to the local database only. Federation is handled by PostgreSQL triggers that call `queue_federation_job()`, which uses `pg_notify` to bridge events into the BullMQ (Redis-backed) job queue consumed by the federation backend. This ensures local operations are fast and federation failures don't block the user.
