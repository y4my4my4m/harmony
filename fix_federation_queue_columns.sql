-- Fix Federation Queue Column Names
-- This fixes the "activity_uuid does not exist" error by using correct column names

BEGIN;

-- =====================================================
-- FIX: Update handle_outgoing_messages with CORRECT column names
-- =====================================================
    ...
    IF NEW.conversation_id IS NOT NULL AND sender_profile.is_local THEN
        ...
        IF v_instance_domain IS NOT NULL AND v_instance_domain != 'debug' THEN
            ...
            LOOP
                ...
                RAISE NOTICE '✅ DM federation queued: %@% -> %@% (activity: %)', 
...
'FIXED: Uses correct federation_delivery_queue column names (activity_id, activity_data, target_inbox_url) to match actual table schema.';
...
DO $$
BEGIN
    RAISE NOTICE '✅ Fixed federation_delivery_queue column names:';
    RAISE NOTICE '   - activity_uuid → activity_id';
    RAISE NOTICE '   - activity → activity_data';  
    RAISE NOTICE '   - target_inbox → target_inbox_url';
    RAISE NOTICE '🚀 Outgoing DMs should now work without column errors!';
END $$;
