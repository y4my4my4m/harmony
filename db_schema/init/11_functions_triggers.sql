...
-- Home-feed realtime push: piggybacks on the existing
-- `create_comprehensive_timeline_entries()` fan-out so we don't repeat the
-- followers SELECT. `broadcast_post_event` only sends `post:new` to the
-- AUTHOR's channel; this one sends `home_feed:new_post` to every recipient
-- whose `home` timeline received the entry (followers + author).
-- See `db_schema/migrations/20260528_home_feed_realtime.sql` for context.
