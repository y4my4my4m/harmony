-- Message link preview processing (direct backend calls)
-- Applies on top of TRUE_LATEST_DB_SCHEMA_BACKUP.sql
--
-- Flow:
-- 1. Harmony-local URLs are enriched directly within Postgres (fast, no HTTP).
-- 2. External URLs are proxied through the federated backend via the `http` extension.

drop function if exists public.process_local_link_previews() cascade;
drop function if exists public.webhook_external_link_previews() cascade;
drop function if exists public.update_message_embeds(uuid, jsonb) cascade;
