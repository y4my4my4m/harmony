...
CREATE POLICY "Users can create post interactions on posts they can see" ON public.post_interactions FOR INSERT WITH CHECK (((user_id = ( SELECT auth.uid() AS uid)) AND (post_id IN ( SELECT p.id
   FROM public.posts p
  WHERE ((p.author_id = ( SELECT auth.uid() AS uid)) OR (p.visibility = 'public'::text) OR ((p.visibility = 'followers'::text) AND (EXISTS ( SELECT 1
           FROM public.follows f
          WHERE ((f.follower_id = ( SELECT auth.uid() AS uid)) AND (f.following_id = p.author_id) AND (f.status = 'accepted'::text))))))))));
...
CREATE POLICY "Users can create their own posts" ON public.posts FOR INSERT WITH CHECK ((( SELECT auth.uid() AS uid) = author_id));
...
CREATE POLICY "Users can view posts from users they follow" ON public.posts FOR SELECT USING (((visibility = 'followers'::text) AND (is_deleted = false) AND (EXISTS ( SELECT 1
   FROM public.follows
  WHERE ((follows.follower_id = ( SELECT auth.uid() AS uid)) AND (follows.following_id = posts.author_id) AND (follows.status = 'accepted'::text))))));
...
CREATE POLICY "Users can view their own posts" ON public.posts FOR SELECT USING ((( SELECT auth.uid() AS uid) = author_id));
