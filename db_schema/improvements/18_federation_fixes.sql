-- Migration: Federation Fixes
-- Fixes issues that broke federation since 11/27
-- 
-- ISSUES FIXED:
-- 1. is_author_suspended() returning NULL for federated users (breaks RLS policies)
-- 2. Missing trigger_post_federation for federating new posts
-- 3. RLS policies blocking legitimate operations due to NULL handling
-- 4. Post disappearing after reaction due to RLS policy issues

-- =====================================================
-- FIX 1: is_author_suspended function returning NULL
-- The old function returns NULL if no profile is found,
-- which causes NOT NULL = NULL in RLS policies
-- =====================================================
...
COMMENT ON FUNCTION public.is_author_suspended(p_author_id uuid) IS 
  'Returns true if the post author is suspended. Returns FALSE for missing profiles (federated content). Used by RLS policies.';


-- =====================================================
-- FIX 2: Post Federation Trigger (was never applied)
-- This trigger federates new local posts to remote instances
-- =====================================================

-- Build ActivityPub Create activity for a post
...
-- Trigger function for post federation
...
-- =====================================================
-- FIX 3: RLS policies need COALESCE for safety
-- Recreate the policies with proper NULL handling
-- =====================================================

-- Drop and recreate "Users can view public posts" policy
...
COMMENT ON POLICY "Users can view public posts" ON public.posts IS 
    'Anyone can view public/unlisted posts that are not deleted and author is not suspended. Safe for federated content.';


-- Drop and recreate "Users can view their own posts" policy  
...
COMMENT ON POLICY "Users can view their own posts" ON public.posts IS 
    'Users can view their own posts only if not deleted. Removed suspension check since users need to see their own posts.';


-- Drop and recreate "Users can view posts from users they follow" policy
...
COMMENT ON POLICY "Users can view posts from users they follow" ON public.posts IS 
    'Users can view follower-only posts from followed accounts, if not deleted and author not suspended. Safe for federated content.';


-- Drop and recreate "Users can create post interactions on posts they can see" policy
...
WITH CHECK (
    ...
    AND post_id IN (
        ...
        AND COALESCE(public.is_author_suspended(p.author_id), false) = false
        AND (
            p.author_id = public.get_current_profile_id() 
            OR p.visibility = ANY (ARRAY['public'::text, 'unlisted'::text]) 
            OR (
                p.visibility = 'followers'::text 
                AND EXISTS (
...
-- Drop and recreate "Users can view post interactions on posts they can see" policy
...
-- Drop and recreate "Users can view their timeline entries" policy
...
GRANT EXECUTE ON FUNCTION public.is_author_suspended(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.is_author_suspended(uuid) TO anon;
...
DO $$
BEGIN
    RAISE LOG 'Migration 18_federation_fixes.sql applied successfully';
