...
COMMENT ON FUNCTION public.is_author_suspended(p_author_id uuid) IS 'Returns true if the post author is suspended. Used by RLS policies.';
...
CREATE POLICY "Users can create post interactions on posts they can see" ON public.post_interactions FOR INSERT WITH CHECK (((user_id = public.get_current_profile_id()) AND (post_id IN ( SELECT p.id
   FROM public.posts p
  WHERE (((p.is_deleted = false) OR (p.is_deleted IS NULL)) AND (NOT public.is_author_suspended(p.author_id)) AND ((p.author_id = public.get_current_profile_id()) OR (p.visibility = ANY (ARRAY['public'::text, 'unlisted'::text])) OR ((p.visibility = 'followers'::text) AND (EXISTS ( SELECT 1
           FROM public.follows f
...
CREATE POLICY "Users can view posts from users they follow" ON public.posts FOR SELECT USING (((visibility = 'followers'::text) AND ((is_deleted = false) OR (is_deleted IS NULL)) AND (NOT public.is_author_suspended(author_id)) AND (EXISTS ( SELECT 1
   FROM public.follows
...
COMMENT ON POLICY "Users can view posts from users they follow" ON public.posts IS 'Users can view follower-only posts from followed accounts, if not deleted and author not suspended';
...
CREATE POLICY "Users can view public posts" ON public.posts FOR SELECT USING (((visibility = ANY (ARRAY['public'::text, 'unlisted'::text])) AND ((is_deleted = false) OR (is_deleted IS NULL)) AND (NOT public.is_author_suspended(author_id))));
...
COMMENT ON POLICY "Users can view public posts" ON public.posts IS 'Anyone can view public/unlisted posts that are not deleted and author is not suspended';
...
CREATE POLICY "Users can view their own posts" ON public.posts FOR SELECT USING (((author_id = public.get_current_profile_id()) AND ((is_deleted = false) OR (is_deleted IS NULL)) AND (NOT public.is_author_suspended(author_id))));
...
COMMENT ON POLICY "Users can view their own posts" ON public.posts IS 'Users can view their own posts only if not deleted and not suspended';
...
CREATE POLICY "Users can view their timeline entries" ON public.timeline_entries FOR SELECT USING (((user_id = public.get_current_profile_id()) AND (post_id IN ( SELECT p.id
   FROM public.posts p
  WHERE (((p.is_deleted = false) OR (p.is_deleted IS NULL)) AND (NOT public.is_author_suspended(p.author_id)))))));
