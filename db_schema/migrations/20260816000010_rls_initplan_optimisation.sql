...
-- Wrapped per statement because ALTER POLICY has no IF EXISTS and the policy set
-- is not identical everywhere. This list was generated from a fresh init/ build;
-- an instance that predates a rename carries the same rule under another name,
-- and one that predates a table rewrite carries a policy whose expression names
-- columns that build does not have -- megolm_room_sessions gained session_id and
-- creator_user_id in 20260530000010, which production and a local dev database
-- never received.
--
-- Rewriting an expression is a performance change and nothing else, so where it
-- does not apply the policy keeps the wording it already has and the run
-- continues. Each skip is named, so the notices list what went unoptimised.
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admin audit log admin access" ON public.admin_audit_log';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can create their own activities" ON public.ap_activities';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can update their own activities" ON public.ap_activities';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can view their own activities" ON public.ap_activities
        USING ((actor_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view their own activities" ON public.ap_activities';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Blocked instances admin access" ON public.blocked_instances
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Blocked instances admin access" ON public.blocked_instances';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'bot_audit_log_select_owner ON public.bot_audit_log';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'bot_commands_modify_owner ON public.bot_commands';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'bot_presence_modify_owner ON public.bot_presence';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'bot_rate_limits_owner_only ON public.bot_rate_limits';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'bot_webhooks_owner_only ON public.bot_webhooks';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channel_categories_delete ON public.channel_categories';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY channel_categories_insert ON public.channel_categories
      ...
      WHERE ((servers.id = channel_categories.server_id) AND (servers.owner = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channel_categories_insert ON public.channel_categories';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channel_categories_update ON public.channel_categories';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY channel_permission_overrides_modify ON public.channel_permission_overrides
      ...
      WHERE ((c.id = channel_permission_overrides.channel_id) AND ((s.owner = public.get_current_profile_id()) OR COALESCE(p.is_admin, false) OR COALESCE(p.is_moderator, false) OR (EXISTS ( SELECT 1
              ...
              WHERE ((ur.user_id = public.get_current_profile_id()) AND (ur.server_id = s.id) AND (((sr.permissions & ((1)::bigint << 0)) <> 0) OR ((sr.permissions & ((1)::bigint << 2)) <> 0) OR ((sr.permissions & ((1)::bigint << 3)) <> 0))))))))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channel_permission_overrides_modify ON public.channel_permission_overrides';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channel_permission_overrides_select ON public.channel_permission_overrides';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channels_delete_owner ON public.channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY channels_insert_owner ON public.channels
      ...
      WHERE ((servers.id = channels.server_id) AND (servers.owner = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channels_insert_owner ON public.channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY channels_select_member ON public.channels
      ...
      WHERE ((servers.id = channels.server_id) AND (servers.owner = public.get_current_profile_id()))))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channels_select_member ON public.channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY channels_update_owner ON public.channels
      ...
      WHERE ((servers.id = channels.server_id) AND (servers.owner = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'channels_update_owner ON public.channels';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_encryption_settings_modify ON public.conversation_encryption_settings';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_encryption_settings_select ON public.conversation_encryption_settings';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_participants_delete_policy ON public.conversation_participants';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_participants_insert_self_owned ON public.conversation_participants';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_participants_select_policy ON public.conversation_participants';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY conversation_participants_update_policy ON public.conversation_participants
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversation_participants_update_policy ON public.conversation_participants';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'conversations_select_participant ON public.conversations';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'device_approval_requests_insert_own ON public.device_approval_requests';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY device_approval_requests_select_own ON public.device_approval_requests
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'device_approval_requests_select_own ON public.device_approval_requests';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Server owners manage discord bridge pairing" ON public.discord_bridge_pairings
      ...
      WHERE ((s.id = discord_bridge_pairings.server_id) AND ((s.owner = public.get_current_profile_id()) OR public.has_permission(public.get_current_profile_id(), s.id, 'MANAGE_SERVER'::text))))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Server owners manage discord bridge pairing" ON public.discord_bridge_pairings';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can delete own emoji favorites" ON public.emoji_favorites
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can delete own emoji favorites" ON public.emoji_favorites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can insert own emoji favorites" ON public.emoji_favorites
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can insert own emoji favorites" ON public.emoji_favorites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can view own emoji favorites" ON public.emoji_favorites
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view own emoji favorites" ON public.emoji_favorites';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emoji_usage_access_policy ON public.emoji_usage';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_delete_server_owner ON public.emojis';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_delete_user_scope ON public.emojis';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY emojis_insert_server_owner ON public.emojis
      ...
      WHERE ((servers.id = emojis.server_id) AND (servers.owner = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_insert_server_owner ON public.emojis';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_insert_user_scope ON public.emojis';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY emojis_update_server_owner ON public.emojis
      ...
      WHERE ((servers.id = emojis.server_id) AND (servers.owner = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_update_server_owner ON public.emojis';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY emojis_update_user_scope ON public.emojis
        USING (((scope = 'user'::text) AND (uploader = public.get_current_profile_id())));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'emojis_update_user_scope ON public.emojis';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY encryption_audit_log_insert_system ON public.encryption_audit_log
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'encryption_audit_log_insert_system ON public.encryption_audit_log';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'encryption_audit_log_own_or_admin ON public.encryption_audit_log';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'encryption_sessions_own_user ON public.encryption_sessions';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY federated_instances_manage ON public.federated_instances
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'federated_instances_manage ON public.federated_instances';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admins can delete federation deliveries" ON public.federation_delivery_queue';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Admins can delete dead endpoints" ON public.federation_endpoint_health
      ...
      WHERE ((profiles.id = public.get_current_profile_id()) AND (profiles.is_admin = true)))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admins can delete dead endpoints" ON public.federation_endpoint_health';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY federation_health_manage ON public.federation_health
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'federation_health_manage ON public.federation_health';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY federation_health_modify_admin ON public.federation_health
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'federation_health_modify_admin ON public.federation_health';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'follows_delete_own ON public.follows';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'follows_insert_own ON public.follows';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'follows_update_involved ON public.follows';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can delete own gif favorites" ON public.gif_favorites
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can delete own gif favorites" ON public.gif_favorites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can insert own gif favorites" ON public.gif_favorites
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can insert own gif favorites" ON public.gif_favorites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can view own gif favorites" ON public.gif_favorites
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view own gif favorites" ON public.gif_favorites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY instance_config_select_admin ON public.instance_config
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'instance_config_select_admin ON public.instance_config';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY donation_history_modify_admin ON public.instance_donation_history
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'donation_history_modify_admin ON public.instance_donation_history';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'donation_history_select_admin ON public.instance_donation_history';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY funding_modify_admin ON public.instance_funding
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'funding_modify_admin ON public.instance_funding';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY pending_donations_admin_all ON public.instance_pending_donations
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'pending_donations_admin_all ON public.instance_pending_donations';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY tiers_modify_admin ON public.instance_supporter_tiers
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'tiers_modify_admin ON public.instance_supporter_tiers';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY supporters_modify_admin ON public.instance_supporters
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'supporters_modify_admin ON public.instance_supporters';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'invites_delete_creator ON public.invites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY invites_select_creator ON public.invites
        USING ((created_by = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'invites_select_creator ON public.invites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY invites_select_instance_admin ON public.invites
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'invites_select_instance_admin ON public.invites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY invites_update_creator ON public.invites
        USING ((created_by = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'invites_update_creator ON public.invites';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY megolm_key_backups_own_only ON public.megolm_key_backups
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_key_backups_own_only ON public.megolm_key_backups';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_key_requests_fulfill ON public.megolm_key_requests';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_key_requests_own ON public.megolm_key_requests';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_key_requests_select_for_response ON public.megolm_key_requests';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_room_sessions_insert_own ON public.megolm_room_sessions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_room_sessions_select ON public.megolm_room_sessions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_room_sessions_update_own ON public.megolm_room_sessions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_session_shares_insert ON public.megolm_session_shares';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_session_shares_select ON public.megolm_session_shares';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'megolm_session_shares_update ON public.megolm_session_shares';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'message_search_index_channel_access ON public.message_search_index';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'message_search_index_conversation_access ON public.message_search_index';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'messages_delete_authorized ON public.messages';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'messages_insert_member ON public.messages';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'messages_select_channel_member ON public.messages';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY messages_update_authorized ON public.messages
        USING (((user_id = public.get_current_profile_id()) OR public.is_current_user_admin() OR public.is_current_user_moderator() OR public.can_current_user_manage_messages_in_channel(channel_id)));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'messages_update_authorized ON public.messages';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY mfa_recovery_codes_own_only ON public.mfa_recovery_codes
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'mfa_recovery_codes_own_only ON public.mfa_recovery_codes';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_channels_delete_own ON public.notification_channels
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_channels_delete_own ON public.notification_channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_channels_insert_own ON public.notification_channels
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_channels_insert_own ON public.notification_channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_channels_select_own ON public.notification_channels
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_channels_select_own ON public.notification_channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_channels_update_own ON public.notification_channels
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_channels_update_own ON public.notification_channels';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_preferences_delete_own ON public.notification_preferences
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_preferences_delete_own ON public.notification_preferences';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_preferences_insert_own ON public.notification_preferences
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_preferences_insert_own ON public.notification_preferences';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_preferences_select_own ON public.notification_preferences
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_preferences_select_own ON public.notification_preferences';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notification_preferences_update_own ON public.notification_preferences
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_preferences_update_own ON public.notification_preferences';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notification_rate_limits_admin_only ON public.notification_rate_limits';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notifications_delete_own ON public.notifications
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notifications_delete_own ON public.notifications';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notifications_select_own ON public.notifications
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notifications_select_own ON public.notifications';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY notifications_update_own ON public.notifications
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'notifications_update_own ON public.notifications';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Admins can read metrics" ON public.performance_metrics
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admins can read metrics" ON public.performance_metrics';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY performance_metrics_hourly_modify_admin ON public.performance_metrics_hourly
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'performance_metrics_hourly_modify_admin ON public.performance_metrics_hourly';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY post_interactions_delete_own ON public.post_interactions
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'post_interactions_delete_own ON public.post_interactions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'post_interactions_insert_own ON public.post_interactions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'posts_delete_own ON public.posts';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'posts_insert_own ON public.posts';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'posts_select_public ON public.posts';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY posts_update_own ON public.posts
        USING ((author_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'posts_update_own ON public.posts';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY reactions_delete_own ON public.reactions
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'reactions_delete_own ON public.reactions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'reactions_insert_own ON public.reactions';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY recovery_key_metadata_own_only ON public.recovery_key_metadata
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'recovery_key_metadata_own_only ON public.recovery_key_metadata';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY remote_emojis_cache_admin_modify ON public.remote_emojis_cache
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'remote_emojis_cache_admin_modify ON public.remote_emojis_cache';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Admins can update reports" ON public.reports
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admins can update reports" ON public.reports';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Admins can view all reports" ON public.reports
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Admins can view all reports" ON public.reports';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can create reports" ON public.reports';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view own reports" ON public.reports';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_bans_select_moderator ON public.server_bans';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_encryption_settings_modify ON public.server_encryption_settings';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_encryption_settings_select ON public.server_encryption_settings';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_folders_delete_own ON public.server_folders
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_folders_delete_own ON public.server_folders';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_folders_insert_own ON public.server_folders
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_folders_insert_own ON public.server_folders';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_folders_select_own ON public.server_folders
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_folders_select_own ON public.server_folders';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_folders_update_own ON public.server_folders
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_folders_update_own ON public.server_folders';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Members can view server membership events" ON public.server_membership_events
      ...
      WHERE (us.user_id = public.get_current_profile_id()))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Members can view server membership events" ON public.server_membership_events';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_roles_delete ON public.server_roles';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_roles_insert ON public.server_roles
      ...
      WHERE ((servers.id = server_roles.server_id) AND (servers.owner = public.get_current_profile_id())))) OR public.is_current_user_admin()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_roles_insert ON public.server_roles';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY server_roles_update ON public.server_roles
      ...
      WHERE ((servers.id = server_roles.server_id) AND (servers.owner = public.get_current_profile_id())))) OR public.is_current_user_admin())));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_roles_update ON public.server_roles';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_settings_modify_owner ON public.server_settings';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'server_settings_select_member ON public.server_settings';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY slow_queries_admin_only ON public.slow_queries
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'slow_queries_admin_only ON public.slow_queries';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY thread_members_delete_self ON public.thread_members
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'thread_members_delete_self ON public.thread_members';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY thread_members_insert_self ON public.thread_members
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'thread_members_insert_self ON public.thread_members';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY thread_members_update_self ON public.thread_members
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'thread_members_update_self ON public.thread_members';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'threads_delete_authorized ON public.threads';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'threads_insert_member ON public.threads';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY threads_update_authorized ON public.threads
      ...
      WHERE ((c.id = threads.channel_id) AND (s.owner = public.get_current_profile_id())))) OR public.is_current_user_admin()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'threads_update_authorized ON public.threads';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY timeline_entries_select_own ON public.timeline_entries
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'timeline_entries_select_own ON public.timeline_entries';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY trending_posts_admin_modify ON public.trending_posts
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'trending_posts_admin_modify ON public.trending_posts';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY trending_refresh_queue_admin_only ON public.trending_refresh_queue
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'trending_refresh_queue_admin_only ON public.trending_refresh_queue';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY trending_users_admin_modify ON public.trending_users
        USING (public.is_current_user_admin());
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'trending_users_admin_modify ON public.trending_users';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY unread_counts_select_own ON public.unread_counts
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'unread_counts_select_own ON public.unread_counts';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY unread_counts_update_own ON public.unread_counts
        ...
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'unread_counts_update_own ON public.unread_counts';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_blocks_check_if_blocked ON public.user_blocks';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_blocks_delete_own ON public.user_blocks';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_blocks_insert_own ON public.user_blocks';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_blocks_select_own ON public.user_blocks
        USING ((blocker_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_blocks_select_own ON public.user_blocks';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_devices_delete_own ON public.user_devices
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_devices_delete_own ON public.user_devices';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_devices_insert_own ON public.user_devices
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_devices_insert_own ON public.user_devices';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_devices_select_own ON public.user_devices
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_devices_select_own ON public.user_devices';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_devices_update_own ON public.user_devices
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_devices_update_own ON public.user_devices';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_list_members_delete ON public.user_list_members';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_list_members_insert ON public.user_list_members
      ...
      WHERE ((ul.id = user_list_members.list_id) AND (ul.user_id = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_list_members_insert ON public.user_list_members';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_list_members_own_list ON public.user_list_members
      ...
      WHERE ((ul.id = user_list_members.list_id) AND (ul.user_id = public.get_current_profile_id())))));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_list_members_own_list ON public.user_list_members';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_lists_delete ON public.user_lists
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_lists_delete ON public.user_lists';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_lists_insert ON public.user_lists
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_lists_insert ON public.user_lists';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_lists_own_select ON public.user_lists
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_lists_own_select ON public.user_lists';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_lists_update ON public.user_lists
        ...
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_lists_update ON public.user_lists';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_mutes_delete_own ON public.user_mutes';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_mutes_insert_own ON public.user_mutes';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_mutes_select_own ON public.user_mutes
        USING ((muter_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_mutes_select_own ON public.user_mutes';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_mutes_update_own ON public.user_mutes
        USING ((muter_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_mutes_update_own ON public.user_mutes';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_roles_delete ON public.user_roles';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_roles_insert ON public.user_roles
      ...
      WHERE ((servers.id = user_roles.server_id) AND (servers.owner = public.get_current_profile_id())))) OR public.is_current_user_admin()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_roles_insert ON public.user_roles';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can leave servers" ON public.user_servers';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_servers_insert_self ON public.user_servers
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_servers_insert_self ON public.user_servers';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_servers_select_co_members ON public.user_servers';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_servers_update_own_or_owner ON public.user_servers';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can delete own sessions" ON public.user_sessions';
END $do$;
...
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can insert own sessions" ON public.user_sessions';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can update own sessions" ON public.user_sessions
        USING ((public.get_current_profile_id() = user_id));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can update own sessions" ON public.user_sessions';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can view own sessions" ON public.user_sessions
        USING ((public.get_current_profile_id() = user_id));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view own sessions" ON public.user_sessions';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can access own timeline cache" ON public.user_timeline_cache
        USING ((public.get_current_profile_id() = user_id));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can access own timeline cache" ON public.user_timeline_cache';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY user_view_contexts_own_user ON public.user_view_contexts
        ...
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'user_view_contexts_own_user ON public.user_view_contexts';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY voice_participants_delete_self ON public.voice_channel_participants
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'voice_participants_delete_self ON public.voice_channel_participants';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY voice_participants_insert_self ON public.voice_channel_participants
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'voice_participants_insert_self ON public.voice_channel_participants';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY voice_participants_update_self ON public.voice_channel_participants
        ...
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', 'voice_participants_update_self ON public.voice_channel_participants';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can create their own voice events" ON public.voice_federation_events
        WITH CHECK ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can create their own voice events" ON public.voice_federation_events';
END $do$;
...
DO $do$ BEGIN
    ALTER POLICY "Users can view voice events they're involved in" ON public.voice_federation_events
        USING ((user_id = public.get_current_profile_id()));
EXCEPTION WHEN undefined_object OR undefined_table OR undefined_column THEN
    RAISE NOTICE 'skipped (not applicable here): %', '"Users can view voice events they''re involved in" ON public.voice_federation_events';
END $do$;
