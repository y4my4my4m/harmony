-- Fix federation trigger functions: the init schema versions only set
-- federation_status = 'queued' but never call queue_federation_job(),
-- so outgoing federation (DMs, channel messages, reactions, edits, deletes)
-- never actually gets enqueued. These replacements match production.
BEGIN;
...
-- ==========================================================================
-- 8. Also pick up any messages stuck with wrong federation_status
-- ==========================================================================
-- Messages that were set to 'queued' by the old triggers but never actually
-- enqueued should be reset to 'pending' so the sweep picks them up.
