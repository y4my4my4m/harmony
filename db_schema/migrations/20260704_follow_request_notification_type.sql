...
--
-- handle_unified_notification_processing always emitted 'activitypub_follow'
-- on follows INSERT, even when the row was created with status='pending'
-- (manual approval mode). Recipients saw "started following you" for
-- requests they still had to approve, and the 'activitypub_follow_request'
-- notification type was never produced despite being supported by the
-- frontend formatter and preference gating.
--
-- This migration branches on NEW.status: pending rows now emit
-- 'activitypub_follow_request', accepted rows keep 'activitypub_follow'.
--
-- It also adds an 'activitypub_follow_accepted' notification to the requester
-- when a pending follow transitions to accepted (local followers only —
-- remote followers learn about acceptance via the federated Accept activity).
-- The trigger is extended to fire on UPDATE OF status for this. The new type
-- needs no send_notification changes: it matches the activitypub_% prefix and
-- falls through to the default-allow branch under the master AP toggle.
-- ---------------------------------------------------------------------------
...
-- Fire on status transitions too so pending->accepted notifies the requester
