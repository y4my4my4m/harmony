-- Server-scoped notifications now require current membership ('accepted' user_servers row).
-- Users who left a server no longer get reaction/reply/mention notifications from it.
-- Full replacement of send_notification; the only change is the membership guard
-- after the mute check. Mirrors init/13_functions_rpc_extended.sql.
