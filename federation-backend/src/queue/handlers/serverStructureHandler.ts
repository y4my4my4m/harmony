/**
 * Federation job handlers for server structure: channel CRUD, category CRUD,
 * and server metadata updates (name, icon, description).
 */
...
export async function handleChannelCrudJob(data: FederationJobData): Promise<void> {
  ...
  try {
    ...
    if (type === 'create') {
      ...
      if (channel && !channel.is_remote) {
        // Sweep-based update: old and new are the same row.
      ...
      // Row is gone; no federation_status update.
...
export async function handleCategoryCrudJob(data: FederationJobData): Promise<void> {
  ...
  try {
    // Categories federate through the channel path with type 2.
    const { 
      handleChannelCreated, 
      handleChannelUpdated, 
      handleChannelDeleted 
