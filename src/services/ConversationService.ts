...
export class ConversationService {
  ...
  static async findConversationRoot(postId: string): Promise<string> {
    console.log(`🔍 Finding ActivityPub conversation root for post: ${postId} (O(1) lookup)`);
    ...
    try {
      ...
      if (error) {
        console.error('❌ Error finding ActivityPub conversation root:', error);
        return postId; // Fallback to original post
      }
      
      const rootId = data?.[0]?.root_id || postId;
      console.log(`✅ Found ActivityPub conversation root: ${rootId} (instant lookup)`);
      return rootId;
      
    } catch (error) {
      console.error('❌ Exception finding ActivityPub conversation root:', error);
      return postId; // Fallback to original post
    }
  }
  ...
  static async getConversationThread(conversationRootId: string): Promise<ConversationThread> {
    console.log(`🧵 Loading ActivityPub conversation thread: ${conversationRootId} (indexed lookup)`);
    ...
    try {
      ...
      if (error) {
        console.error('❌ Error loading ActivityPub conversation thread:', error);
        throw error;
      }
      
      const posts = data || [];
      console.log(`✅ Loaded ${posts.length} posts in ActivityPub conversation (fast indexed lookup)`);
      ...
      console.error('❌ Failed to load ActivityPub conversation thread:', error);
      throw error;
    }
  }
  ...
  static async getConversationContext(postId: string) {
    console.log(`📊 Getting ActivityPub conversation context for: ${postId}`);
    ...
    try {
      ...
      if (error) {
        console.error('❌ Error getting ActivityPub conversation context:', error);
        return null;
      }
      
      const context = data?.[0];
      console.log(`✅ Got ActivityPub conversation context:`, context);
      return context;
      
    } catch (error) {
      console.error('❌ Failed to get ActivityPub conversation context:', error);
      return null;
    }
  }
