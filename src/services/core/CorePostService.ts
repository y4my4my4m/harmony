...
export class CorePostService {
  ...
  async updatePost(postId: string, updates: UpdatePostData): Promise<TimelinePost> {
    try {
      ...
      // Length is enforced on edit; otherwise an edit could push a short post
      // past the limit. Checked only when content changes.
      if (updates.content) {
        const maxPostLength = await this.getMaxPostLength()
    ...
    try {
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          debug.log(`ℹCore: Post not found: ${postId}`)
          return null
        }
