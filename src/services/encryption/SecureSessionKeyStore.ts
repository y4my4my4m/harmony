...
class SecureSessionKeyStore {
  ...
  /**
   * Clear stored keys for every user.
   */
...
class SigningKeyStore {
  ...
  async store(userId: string, privateKey: CryptoKey): Promise<void> {
    ...
    // Re-import as non-extractable when the caller passes an extractable key.
    let safeKey = privateKey
    if (privateKey.extractable) {
      const raw = await crypto.subtle.exportKey('pkcs8', privateKey)
      safeKey = await crypto.subtle.importKey(
...
// when a contact's signing identity changes and surface a non-blocking notice.
// Not a Matrix-style mandatory verification wall.
//
// Keyed by userId today; the record carries an optional deviceId so the Phase 3
// per-device identity model can pin per (userId, deviceId); that switch needs a
// DB_VERSION bump and a new keyPath, not a schema migration.
...
// (NOT userId). Each browser profile / install signs its own v3 messages with a
// key that never leaves the device, so revoking a device
// (user_devices.revoked_at) cuts off that device's ability to produce
// verifiable messages.
//
