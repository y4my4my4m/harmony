...
export class RecoveryKeyService {
  ...
  validateMnemonic(words: string[], strict = false): boolean {
    ...
    // Strict mode needs SHA-256, which is async; this signature is sync.
    // Strict callers use validateMnemonicStrict() below. Fails closed here.
    return false
  }
  ...
  async deriveKeysFromMnemonic(words: string[]): Promise<DerivedKeys> {
    // NOTE: validation here is LOOSE (word count + wordlist membership), not
    // the strict BIP39 checksum. Phrases from the legacy sync generator carry
    // no SHA-256 checksum and must stay derivable. Strict checksum failure is
    // surfaced as a non-blocking typo hint in the recovery UI instead, so a
    // valid legacy phrase is never rejected.
