...
export class RecoveryKeyService {
  ...
  async deriveKeysFromMnemonic(words: string[]): Promise<DerivedKeys> {
    // NOTE: We intentionally use LOOSE validation here (word-count + wordlist
    // membership), NOT the strict BIP39 checksum. Derivation must remain
    // backwards-compatible with phrases generated by older clients (the legacy
    // sync generator did not produce a SHA-256 checksum), and the recovery
    // flow's own contract treats any wordlist-valid phrase as derivable.
    // Strict checksum validation is surfaced as a non-blocking typo hint in the
    // recovery UI instead (so a typo can't silently lock a user out, but a valid
    // legacy phrase is never rejected).
