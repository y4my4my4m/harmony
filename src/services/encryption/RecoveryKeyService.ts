...
export class RecoveryKeyService {
  ...
  generateMnemonic(wordCount: 12 | 24 = 12): string[] {
    ...
    const entropyBytes = wordCount === 12 ? 16 : 32 // 16 bytes = 128 bits, 32 bytes = 256 bits
    const entropy = crypto.getRandomValues(new Uint8Array(entropyBytes))

    // Convert entropy to mnemonic
    // Each word represents 11 bits of entropy
    // 128 bits = 12 words (128 / 11 ≈ 11.6, rounded with checksum = 12)
    // 256 bits = 24 words (256 / 11 ≈ 23.3, rounded with checksum = 24)
    ...
    // Add checksum bits (first 4 bits of SHA-256 hash for 12 words, 8 bits for 24 words)
    // For simplicity, we'll use the first bytes of the entropy as pseudo-checksum
    const checksumBits = entropy[0].toString(2).padStart(8, '0').slice(0, wordCount === 12 ? 4 : 8)
